...
export const initialActivities = () => {
  return {
    day0: {
      day: DateUtils.getDayName(0),
      dayString: DateUtils.getFormatStringBySubtract('dddd, MMMM D, YYYY', 0),
      steps: 0,
      calories: 0,
      distances: 0,
    },
    day1: {
      day: DateUtils.getDayName(1),
      dayString: DateUtils.getFormatStringBySubtract('dddd, MMMM D, YYYY', 1),
      steps: 0,
      calories: 0,
      distances: 0,
    },
    day2: {
      day: DateUtils.getDayName(2),
      dayString: DateUtils.getFormatStringBySubtract('dddd, MMMM D, YYYY', 2),
      steps: 0,
      calories: 0,
      distances: 0,
    },
    day3: {
      day: DateUtils.getDayName(3),
      dayString: DateUtils.getFormatStringBySubtract('dddd, MMMM D, YYYY', 3),
      steps: 0,
      calories: 0,
      distances: 0,
    },
    day4: {
      day: DateUtils.getDayName(4),
      dayString: DateUtils.getFormatStringBySubtract('dddd, MMMM D, YYYY', 4),
      steps: 0,
      calories: 0,
      distances: 0,
    },
    day5: {
      day: DateUtils.getDayName(5),
      dayString: DateUtils.getFormatStringBySubtract('dddd, MMMM D, YYYY', 5),
      steps: 0,
      calories: 0,
      distances: 0,
    },
    day6: {
      day: DateUtils.getDayName(6),
      dayString: DateUtils.getFormatStringBySubtract('dddd, MMMM D, YYYY', 6),
      steps: 0,
      calories: 0,
      distances: 0,
    },
...
function getActivitiesByIndex(steps, calories, distances, index) {
  ...
  const filterSteps = steps.filter(item => {
    return (
      DateUtils.getTimeStamp(item.startDate) > startDate &&
...
export const prepareSleepAnalysis = ({sleeps, index}) => {
  ...
  const filterItems = sleeps
    ...
    .map(item => {
      return {
        ...item,
        duration:
          DateUtils.getTimeStamp(item.endDate) -
          DateUtils.getTimeStamp(item.startDate),
      };
    });
  ...
  return {
    ...
    deepPercent:
      sleepSeconds > 0
        ? `${Math.round((deepSeconds / sleepSeconds) * 100)}%`
        : '0%',
    lightPercent:
      sleepSeconds > 0
        ? `${Math.round((lightSeconds / sleepSeconds) * 100)}%`
        : '0%',
    remPercent:
      sleepSeconds > 0
        ? `${Math.round((remSeconds / sleepSeconds) * 100)}%`
        : '0%',
  };
};
