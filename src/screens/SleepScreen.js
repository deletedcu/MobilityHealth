...
import {
  initialSleepAnalysis,
  prepareSleepAnalysis,
} from '../utils/FitnessUtils';
import {
  getFormatStringBySubtract,
...
const SleepScreen = props => {
  ...
  function handleChangeDay(index) {
    setKey(index);
    if (demoAvailable) {
      const value = prepareSleepAnalysis({
        sleeps: tempSleepAnalysis,
        index: index,
      });
      setData(value);
    } else {
    ...
    return (
      <TouchableOpacity onPress={() => handleChangeDay(index)}>
        <Surface
          style={[
            style.surface,
            index === key && {backgroundColor: colors.primary},
          ]}>
          <Text style={index === key && {color: colors.background}}>
            {getFormatStringBySubtract('dd', index)}
          </Text>
        </Surface>
      </TouchableOpacity>
    );
      ...
      <ScrollView style={style.container}>
        <View style={style.container}>
          ...
          <View style={style.mainContainer}>
            <Title>
              {getFormatStringBySubtract('dddd, MMMM D, YYYY', key)}
            </Title>
            ...
            <View style={style.itemContainer}>
              <Card.Title
                ...
                left={props1 => (
                  <Avatar.Icon {...props1} icon="clock-time-five" />
                )}
                right={props1 => (
                  <Text {...props1}>
                    {getTimeStringBySeconds(data.sleepTime)}
                  </Text>
                )}
              />
              <Divider />
              <Card.Title
                ...
                left={props1 => <Avatar.Icon {...props1} icon="leaf" />}
                right={props1 => (
                  <Text {...props1}>
                    {getTimeStringBySeconds(data.lightTime)}
                  </Text>
                )}
              />
              <Divider />
              <Card.Title
                ...
                left={props1 => <Avatar.Icon {...props1} icon="power-sleep" />}
                right={props1 => (
                  <Text {...props1}>
                    {getTimeStringBySeconds(data.deepTime)}
                  </Text>
                )}
              />
              <Divider />
              <Card.Title
                ...
                left={props1 => (
                  <Avatar.Icon {...props1} icon="head-lightbulb" />
                )}
                right={props1 => (
                  <Text {...props1}>
                    {getTimeStringBySeconds(data.remTime)}
                  </Text>
                )}
              />
              <Divider />
              <Card.Title
                ...
                left={props1 => (
                  <Avatar.Icon {...props1} icon="weather-sunset" />
                )}
                right={props1 => (
                  <Text {...props1}>
                    {getTimeStringBySeconds(data.wakeTime)}
                  </Text>
                )}
              />
              <Divider />
              <Card.Title
                ...
                left={props1 => <Avatar.Icon {...props1} icon="bed" />}
                right={props1 => (
                  <Text {...props1}>
                    {getTimeStringBySeconds(data.bedTime)}
                  </Text>
                )}
              />
            </View>
